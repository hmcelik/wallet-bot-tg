...
Username: Final = os.getenv("BOT-USERNAME")
ETHERSCAN_API_KEY: Final = os.getenv("ETHERSCAN-API-KEY")
BSCSCAN_API_KEY: Final = os.getenv("BSCSCAN-API-KEY")
TELEGRAM_BOT_TOKEN: Final = os.getenv("TELEGRAM-BOT-TOKEN")
TELEGRAM_CHAT_ID = '5057224206'
...
async def help(update: Update, context: ContextTypes):
    await update.message.reply_text("My commands are: /start, /help")

async def add(update: Update, context: ContextTypes):
    if len(context.args) < 2:
        await update.message.reply_text("Please provide a blockchain and wallet address to add.")
        return
    ...
    if blockchain == 'eth':
        if not re.match(r'^0x[a-fA-F0-9]{40}$', wallet_address):
            await update.message.reply_text(f"{wallet_address} is not a valid Ethereum wallet address.")
            return
    elif blockchain == 'bnb':
        if not re.match(r'^0x[a-fA-F0-9]{40}$', wallet_address):
            await update.message.reply_text(f"{wallet_address} is not a valid Binance Smart Chain wallet address.")
            return
    else:
        await update.message.reply_text(f"Invalid blockchain specified: {blockchain}")
        return
...
async def remove(update: Update, context: ContextTypes):
    if len(context.args) < 2:
        await update.message.reply_text("Please provide a blockchain and wallet address to remove.\nUsage: /remove ETH 0x123456789abcdef")
        return
    blockchain = context.args[0].lower()
    wallet_address = context.args[1]
...
async def list(update: Update, context: ContextTypes):
